...
export interface GitCommandInfo {
  ...
  canUndo: boolean;
  undoHow?: string;
  examples: { cmd: string; explanation: string }[];
  flags?: { flag: string; meaning: string; risky?: boolean }[];
  tip?: string;
}

export const GIT_COMMANDS: GitCommandInfo[] = [
  {
    command: 'git commit',
...
export const COMMAND_MAP = new Map<string, GitCommandInfo>();
for (const cmd of GIT_COMMANDS) {
  COMMAND_MAP.set(cmd.command, cmd);
  const parts = cmd.command.split(' ');
  if (parts.length >= 2) {
    COMMAND_MAP.set('git ' + parts[1], cmd);
  }
}

export function matchCommand(input: string): GitCommandInfo | undefined {
  const trimmed = input.trim().toLowerCase();

  if (/git\s+push\s+(--force|-f)\b/.test(trimmed)) {
    return COMMAND_MAP.get('git push --force');
  }

  for (const [key] of COMMAND_MAP) {
  ...
  if (match) {
    return COMMAND_MAP.get('git ' + match[1]);
  }
