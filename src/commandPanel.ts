import { GitCommandInfo, RiskLevel, RISK_LABELS } from "./gitCommands";

export function getWebviewContent(info: GitCommandInfo, query: string): string {
  const riskColor = getRiskColor(info.riskLevel);
  const riskLabel = RISK_LABELS[info.riskLevel];
  ...
  const whatHappensList = info.whatHappens
    .map((w) => `<li>${esc(w)}</li>`)
    .join("");

  const examplesHtml = info.examples
    .map(
      (e) => `
    <div class="example">
      <code>${esc(e.cmd)}</code>
  ...
  const warningBanner =
    info.riskLevel === "high" || info.riskLevel === "critical"
...
export function buildCheatSheetContent(commands: GitCommandInfo[]): string {
  const rows = commands
    .map((c) => {
    ...
    <tr>
      ...
      <td>${esc(c.summary)}</td>
      <td style="color:${color}; font-weight:600">${RISK_LABELS[c.riskLevel]}</td>
      <td>${c.canUndo ? "✅" : "❌"}</td>
    </tr>`;
    })
    .join("");
